...
test("WebMCP verifies a visible decision receipt without purchasing or sharing comparison context", async ({ page }) => {
  ...
  expect(result).toMatchObject({ status: "verified", data: { product: { sku: "AX7-BLK", variant: "Black" }, terms: { delivered_total_pence: 47405, delivery_estimate: "Arrives Tuesday" }, privacy: { credentials_shared: false, competitor_data_shared: false, purchase_created: false } } });
  ...
  await expect(receipt).toContainText(/474\.05/);
